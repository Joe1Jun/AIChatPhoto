...
interface PreviousChatProps {
  ...
  const PreviousChat: React.FC<PreviousChatProps> = () => {
    const location = useLocation(); // Access location object
    const conversationId = location.state?.conversationId; // Extract conversationId
    ...
    useEffect(() => {
      console.log("Received conversationId:", conversationId); // Make sure this is the correct value
      const fetchMessages = async () => {
        try {
          const response = await axios.post(`http://localhost:5000/selectedConversation`,{
              conversationId
